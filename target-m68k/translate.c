...
#define DEFF96(name,  offset) do { \
    QREG_##name##H = tcg_global_mem_new_i32(cpu_env,    \
        offsetof(CPUM68KState, offset.l.upper), #name); \
    QREG_##name##L = tcg_global_mem_new_i64(cpu_env,    \
        offsetof(CPUM68KState, offset.l.lower), #name); \
} while (0);
#include "qregs.def"
#undef DEFO32
#undef DEFO64
...
static int gen_ea_mode_FP0(CPUM68KState *env, DisasContext *s, int mode, int reg0,
...
DISAS_INSN(fpu)
{
    ...
    if (ext & (1 << 14)) {
        ...
        opsize = ext_opsize(ext, 10);
        if (gen_ea_FP0(env, s, insn, opsize, EA_LOADU) == -1) {
            gen_addr_fault(s);
            return;
        }
    } else {
    ...
    if (round) {
        gen_reduce_FP0(OS_SINGLE);
        gen_extend_FP0(OS_SINGLE);
    }
    gen_op_store_fpr_FP0(REG(ext, 7));
    return;
undef:
...
static double floatx80_to_double(CPUM68KState *env, floatx80 a)
{
    union {
        float64 f64;
        double d;
                         ...
                         int flags)
{
    ...
    for (i = 0; i < 8; i++)
      {
        cpu_fprintf(f, "D%d = %08x   A%d = %08x   F%d = %04x %016"PRIx64"  (%12g)\n",
                    i, env->dregs[i], i, env->aregs[i],
                    i, env->fregs[i].l.upper, env->fregs[i].l.lower,
                    floatx80_to_double(env, env->fregs[i].d));
      }
    ...
    cpu_fprintf (f, "FPRESULT = %04x %016"PRIx64"  (%12g)\n",
                 env->fp0.l.upper, env->fp0.l.lower,
                 floatx80_to_double(env, env->fp0.d));
}
